...
/* array in the code)		*/
{
	...
	*pushsp = pptr->pesp = (unsigned long) saddr;
	...
	for (i = 0; i < NUM_BS; i++) {		
		pptr->bs_map[i].bs_status = BSM_UNMAPPED;
		pptr->bs_map[i].bs_vpno = -1;
		pptr->bs_map[i].bs_npages = 0;
		pptr->bs_map[i].bs_ref = 0;
		pptr->bs_map[i].bs_isPriv = 0;
	}
	...
	if (create_PD(pid) == SYSERR) {
		//kprintf("ERROR:create:failed to create page directory");
		restore(ps);
		return SYSERR;
	}
	restore(ps);
